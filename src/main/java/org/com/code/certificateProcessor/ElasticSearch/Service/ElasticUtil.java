...
public class ElasticUtil {
     ...
     * @throws IOException
     */
    public void bulkIndex(List<StandardAwardRequest> documents, String indexName,List<float[]> vectorList) throws IOException {
        ...
        for (int i = 0; i < documents.size(); i++) {
            ...
            documents.get(i).setActive(true);
        }

        // 1. 创建一个BulkOperation列表
        List<BulkOperation> bulkOperations = new ArrayList<>();

        for (StandardAwardRequest doc : documents) {
            // 为每个文档创建一个 "index" 操作
    ...
    public void bulkUpdate(List<StandardAwardRequest> documentsToUpdate, String indexName, List<float[]> vectorList) throws IOException {
        ...
        for (StandardAwardRequest doc : documentsToUpdate) {
            BulkOperation operation = BulkOperation.of(op -> op
                    .update(idx -> idx
                            .index(indexName)
                            .id(doc.getStandardAwardId()) // 这里需要一个唯一的ID
