...
CREATE TABLE `standard_award` (
                                  ...
                                  `score` DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '此奖项对应的分数',
                                  `isActive` BOOLEAN NOT NULL DEFAULT TRUE COMMENT '是否启用 (可用于软删除或临时禁用某个奖项)',
                                  `createdBy` BIGINT COMMENT '创建此奖项的管理员ID',
                                  `updatedBy` BIGINT COMMENT '最后修改此奖项的管理员ID',
                                  `createdAt` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                  `updatedAt` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                  PRIMARY KEY (`id`),
...
CREATE TABLE `award_submission` (
                                    ...
                                    `finalScore` bigint COMMENT '最终获得的分数 (从standard_awards复制而来)',
                                    `rejectionReason` VARCHAR(500) COMMENT '驳回原因 (AI或管理员填写)',
                                    `aiSuggestion` JSON COMMENT 'AI辅助信息，如ES检索的Top5候选列表，供管理员参考',
                                    `duplicateCheckResult` ENUM('NOT_CHECKED', 'IS_DUPLICATE', 'NOT_DUPLICATE') DEFAULT 'NOT_CHECKED' COMMENT 'LLM查重结果',
                                    `reviewedBy` BIGINT COMMENT '进行人工审核的管理员ID',
